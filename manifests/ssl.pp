...
define ds_389::ssl(
  ...
  exec { "Import ssl ldif: ${name}":
    command => "cat /etc/dirsrv/slapd-${name}/ssl.ldif | ldapmodify -h ${server_host} -p ${server_port} -x -D \"${root_dn}\" -w ${root_dn_pass} ; touch /etc/dirsrv/slapd-${name}/ssl.done", # lint:ignore:140chars
    path    => '/usr/bin:/bin',
