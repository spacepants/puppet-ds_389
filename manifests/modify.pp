...
define ds_389::modify(
  ...
  exec { "Modify ldif ${name}: ${server_id}":
    command => "ldapmodify -h ${server_host} -p ${server_port} -x -D \"${root_dn}\" -w ${root_dn_pass} -f /etc/dirsrv/slapd-${server_id}/${name}.ldif ; touch /etc/dirsrv/slapd-${server_id}/${name}.done", # lint:ignore:140chars
    path    => '/usr/bin:/bin',
